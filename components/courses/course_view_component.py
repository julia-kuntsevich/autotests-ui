...
class CourseViewComponent(BaseComponent):
    ...
    def check_visible(
        ...
        expect(self.max_score_text).to_have_text(f'Max score: {max_score}')

        expect(self.min_score_text.nth(index)).to_be_visible()
        expect(self.min_score_text).to_have_text(f'Min score: {min_score}')

        expect(self.estimated_time_text.nth(index)).to_be_visible()
        expect(self.estimated_time_text).to_have_text(f'Estimated time: {estimated_time}')